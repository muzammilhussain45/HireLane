...
const FindJobPage = () => {
  ...
  const toggleSaveJob = async (jobId) => {
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/saved/job/${jobId}`, {
        method: "POST",
        headers: {
  ...
  const confirmApply = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${BACKEND_URL}/api/application/apply/${confirmToast.jobId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    const fetchAppliedJobs = async () => {
      try {
        ...
        const res = await fetch(`${BACKEND_URL}/api/application/user`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${BACKEND_URL}/api/job?${params.toString()}`,
      );
      const data = await res.json();
  ...
  const getCategoryColor = (category) => {
    const colors = {
      Engineering: "bg-[#d7e9f5] text-[#07508b] border-[#126db3]/25",
      IT: "bg-[#eef4e9] text-[#36566a] border-[#071522]/12",
      "Data Science": "bg-[#d7e9f5] text-[#07508b] border-[#126db3]/25",
      ...
      Finance: "bg-[#d7e9f5] text-[#07508b] border-[#126db3]/25",
    };
    return colors[category] || "bg-[#eef4e9] text-[#36566a] border-[#071522]/12";
  };

  const getExperienceColor = (level) => {
    const colors = {
      Junior: "bg-green-100 text-green-700 border-green-200",
      "Mid-Level": "bg-[#d7e9f5] text-[#07508b] border-[#126db3]/25",
      "Mid-Senior": "bg-[#d7e9f5] text-[#07508b] border-[#126db3]/25",
      Senior: "bg-yellow-100 text-yellow-700 border-yellow-200",
    };
    return colors[level] || "bg-[#eef4e9] text-[#36566a] border-[#071522]/12";
  };

  const getLocationColor = (location) =>
    "bg-[#d7e9f5] text-[#07508b] border-[#126db3]/25";

  const getJobTypeColor = (type) => {
    if (!type) return "bg-[#eef4e9] text-[#36566a] border-[#071522]/12";
    ...
    return (
      colors[normalizedType] || "bg-[#eef4e9] text-[#36566a] border-[#071522]/12"
    );
  };
  ...
  const renderLogo = (job) => {
    ...
    const isBroken =
      imgErrors[job.id] || !logoUrl || logoUrl === BACKEND_URL;

    const initials = job.company?.charAt(0) || "?";
    const colors = [
      "bg-linear-to-br from-[#e8f2f9]0 to-[#07508b]",
      "bg-linear-to-br from-[#e8f2f9]0 to-pink-600",
          ...
          <div className={s.sidebar} ref={filtersRef}>
            <div className={s.sidebarSticky}>
              <div className={s.filterCard}>
                ...
                <div className={s.filterSection}>
                  <h3 className={s.filterSectionTitle}>
                    <BadgeDollarSign className={s.filterSectionIcon} />
                    Salary Range
                  </h3>
