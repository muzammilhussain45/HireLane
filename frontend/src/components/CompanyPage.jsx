...
const STORAGE_KEY = "savedQuestionsIds";
...
const CompanyPage = () => {
  ...
  // Fetch Companies & Saved IDs Effect
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        const response = await axios.get(
          `${BACKEND_URL}/api/interview/companies`,
        );
    ...
    const fetchSavedIds = async () => {
      try {
        const rawUser = localStorage.getItem("hirelane_user");
        ...
        console.error("Error fetching the saved questions", error);
      }
    };
  ...
  // Fetch Selected Company Questions Effect
  useEffect(() => {
    ...
    const fetchQuestions = async () => {
      ...
      try {
        const response = await axios.get(
          `${BACKEND_URL}/api/interview/company/${selectedCompany}`,
        );
        if (response.data.success) {
  ...
  const toggleSave = async (id, openSaved = false) => {
    try {
      ...
      const res = await fetch(
        `${BACKEND_URL}/api/saved/question/${id}?type=interview`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
            ...
            {companies.map((company) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`${s.tabText} ${
                      isSelected ? s.tabTextSelected : s.tabTextUnselected
                    }`}
                    style={{ maxWidth: 140 }}
                  >
                    {company.companyName}
          ...
          <>
            <div className={s.questionsGrid}>
              {questionsToShow.map((question, index) => {
                ...
                const points = Array.isArray(question.keyPoints)
                  ? question.keyPoints
                  : question.keyPoints
                    ? [question.keyPoints]
                    : [];
