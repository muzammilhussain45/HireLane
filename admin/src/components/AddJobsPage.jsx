    ...
    if (validateForm()) {
      try {
        ...
        if (response.data.success) {
          ...
          setFormData({ ...initialFormState, postedDate: getTodayDate() });
          setErrors({});
        }
      } catch (error) {
    ...
    <div className={s.pageContainer}>
      ...
      <div className={s.contentWrapper}>
        ...
        <form onSubmit={handleSubmit} className={s.formCard}>
          ...
          {/* --------- tech stack (array field) --------- */}
          <div className={s.arraySection}>
            <label className={s.arrayLabel}>
              <Code2 size={16} className={s.uploadIcon} />
              Tech Stack <span className={s.uploadRequired}>*</span>
            </label>
          ...
          <button
            type="submit"
            disabled={isPosting}
            className={`${s.submitBtn} ${isPosting ? s.submitButtonDisabled : ""}`}
          >
            {isPosting ? (
              <>
