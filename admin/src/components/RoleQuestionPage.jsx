...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div
      className={`fixed top-28 right-4 z-[200] flex items-center gap-3 p-4 border-l-4 rounded-lg shadow-lg ${bg} animate-fade-in-up`}
    >
      <Icon className={`w-5 h-5 ${iconColor}`} />
      <span className="text-[#36566a]">{message}</span>
    </div>
  );
};
...
const RoleQuestionPage = () => {
  ...
  const handleAddRoleQuestion = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${BACKEND_URL}/api/interview/roles`,
        formDataToSend,
        {
          headers: {
      ...
      console.error("Error adding the role question", err);
      setToast({
        message:
          err.response?.data?.message || "Failed to add the role questions",
        type: "error",
      });
    } finally {
  ...
  const handleImageChange = (e) => {
    ...
    // NOTE: the video first wrote f.startsWith(...) and then fixed it to f.type.startsWith(...)
                ...
                {csvFile ? (
                  ...
                  <>
                    ...
                    <p className={s.uploadHint}>
                      ...
                      values. Company may include `Google|2021-03-10`.
                    </p>
                  </>
                )}
                      ...
                      {(q.keyPoints || []).length > 0 && (
                        <div className={s.keyPointsContainer}>
                          {(q.keyPoints || []).map((kp, i) => (
                            <span key={i} className={s.keyPointsBadge}>
                              {kp}
                            </span>
                          ))}
                        </div>
