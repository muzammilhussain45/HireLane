...
//validateForm() form data fields
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${BACKEND_URL}/api/company`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      })
      ...
      setToast({
        type: 'error',
        message: error.response?.data?.message || 'Failed to add the company',
      })
    } finally {
      setIsLoading(false)
    }
  ...
  const handleConfirmDelete = async () => {
    try {
      const token = localStorage.getItem('token')
      await axios.delete(`${BACKEND_URL}/api/company/${pendingDeleteId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
    ...
    <div className={s.pageContainer}>
      ...
      {toast && (
        <div className={s.toastWrapper}>
          <div className={`${s.toastBase} ${toast.type === 'success' ? s.toastSuccess : toast.type === 'error' ? s.toastError : s.toastConfirm}`}>
            {toast.type === 'success'
            ...
            <div className={s.toastContent}>
              ...
              {toast.confirm && (
                <div className={s.toastActionRow}>
                  <button onClick={handleConfirmDelete} className={s.toastConfirmBtn}>Confirm</button>
                  <button onClick={handleCancelDelete} className={s.toastCancelBtn}>Cancel</button>
                </div>
              )}
            </div>
          ...
          <form onSubmit={handleSubmit} className={s.form}>
            ...
            <div className={s.submitSection}>
              <button
                type='submit'
                disabled={isLoading}
                className={`${s.submitBtn} ${isLoading ? s.submitButtonDisabled : ''}`}
              >
                {isLoading ? (
                  <>
