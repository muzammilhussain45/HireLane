...
// helper function to parse count strings like "10k+" or "1600" into numbers
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        console.error("Error adding the interview questions", err);
        setToast({
          type: "error",
          message:
            err.response?.data?.message ||
            "Failed to add the interview questions",
        });
      } finally {
        setIsSubmitting(false);
      }
    } else {
      setToast({ type: "error", message: "Please fill all the required fields" });
    }
  };
    ...
    <div className={s.pageContainer}>
      {toast && (
        <div className={s.toastWrapper}>
          <div
            className={`${s.toastBase} ${toast.type === "success" ? s.toastSuccess : s.toastError}`}
          >
            {toast.type === "success" ? (
            ...
            <button className={s.toastCloseButton} onClick={() => setToast(null)}>
              <X size={16} />
            </button>
          </div>
        </div>
      ...
      <div className={s.contentWrapper}>
        <header className={s.header}>
          ...
          <p className={s.headerSubtitle}>
            Add company details and upload a question bank CSV (question, answer,
            key points, posted date optional)
          </p>
        </header>
          ...
          <form onSubmit={handleSubmit} className={s.form}>
            ...
            <div className={s.gridRow}>
              ...
              <div className={s.logoContainer}>
                ...
                <div>
                  ...
                  />
                  <p className={s.logoHint}>PNG, JPG, GIF up to 2MB (optional)</p>
                </div>
              </div>
              ...
              <div className={s.colSpan2Alt}>
                <label className={s.label}>
                  Upload CSV (question, answer, key points, posted date optional){" "}
            ...
            <div className={s.submitSection}>
              <button
                ...
                className={`${s.submitBtn} ${isSubmitting ? s.submitButtonDisabled : ""}`}
              >
                {isSubmitting ? (
                  <>
                    <Loader2 className={s.submitSpinner} />
                    Adding...
                  </>
                ) : (
                      ...
                      {q.keyPoints && q.keyPoints.length > 0 && (
                        <div className={s.keyPointsSection}>
                          ...
                          <ul className={s.keyPointsList}>
                            {q.keyPoints.map((kp, kIndex) => (
                              <li key={kIndex} className={s.keyPointItem}>
                                <CircleDashed
                                  size={14}
