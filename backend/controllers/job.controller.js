...
export const createJob = async (req, res) => {
    try {
        ...
        let companyLogo = "";
        if (req.file) {
            const uploadRes = await uploadToCloudinary(req.file.buffer, "HireLane/logos", "image", req.file.originalname);
...
// get the dashboard stats for jobs and applications for admin
    ...
    try {
        ...
        return res.status(200).json({
            ...
            stats: {
                ...
                totalApplications: totalApplicationsCount.toLocaleString(),
...
export const getJobsByAdmin = async (req, res) => {
    try {
        ...
        const countMaps = applicationStats.reduce((acc, curr) => {
        ...
        const jobsWithStats = jobs.map(job => {
            ...
            return {
                ...job._doc,
                applicationCount: countMaps[job._id.toString()] || 0
            };
        });
...
export const closeJob = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            ...
            job // Return the updated job object
